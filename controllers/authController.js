...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email }).select('+password'); //+ is used to select a field that is not selected by default
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser)
    return next(
      new AppError(
        'The user for which this token was created no longer exists.',
        401
      )
    );
