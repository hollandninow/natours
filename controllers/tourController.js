  ...
  const stats = await Tour.aggregate([
    ...
    },
    {
      $group: {
        ...
        avgRating: { $avg: '$ratingsAverage' }, // Need $ in front of field on which you want to calculate average or some other operator
        avgPrice: { $avg: '$price' },
        minPrice: { $min: '$price' },
        maxPrice: { $max: '$price' },
      },
  ...
  const distances = await Tour.aggregate([
    {
      $geoNear: {
        near: {
          type: 'Point',
          coordinates: [lng * 1, lat * 1], // lng always first
        },
